...
func TestNewExec(t *testing.T) {
	for ptyp, path := range testdata {
		ex, err := NewExec(path)
		if err != nil {
			t.Errorf("want err=nil; got %q (ptyp=%v)", err, ptyp)
			continue
		}
		if ex.Type != ptyp {
			t.Errorf("want ex.Type=%v; got %v", ptyp, ex.Type)
		}
	}
}
...
func TestImport(t *testing.T) {
	for ptyp, path := range testdata {
		...
		if imp != echo {
			t.Errorf("want imp=%q; got %q (ptyp=%v)", echo, imp, ptyp)
		}
	}
}
